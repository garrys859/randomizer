...
function renderPlaylistView() {
  ...
  const currentTrack = container.querySelector(`.playlist-item[data-index='${currentIndex}']`);
  if (currentTrack) {
    currentTrack.style.backgroundColor = "#e9d3f3"; // Color distintivo para resaltar
    currentTrack.style.borderLeft = "5px solid #a764c5"; // Línea distintiva a la izquierda
  }
}
...
import { savePlaylistToServer, getPlaylistFromServer } from "./api.js";
...
document.getElementById("generateTokenBtn").addEventListener("click", async () => {
  ...
  const playlist = [...videos]; // Obtiene la lista de reproducción actual
  alert(`Tu token único es: ${token}`);
  
  try {
    await savePlaylistToServer(token, playlist); // Envía la playlist al servidor
    console.log(`Playlist guardada con éxito para el token ${token}`);
  } catch (error) {
...
document.getElementById("loadPlaylistBtn").addEventListener("click", async () => {
  const token = prompt("Ingresa el token para cargar tu playlist:");
  if (!token) return;

  try {
    const playlist = await getPlaylistFromServer(token);
    if (playlist) {
      console.log("Playlist cargada:", playlist);
      videos = playlist; // Actualiza la lista actual
      renderPlaylistView();
    } else {
      alert("No se encontró una playlist asociada a este token.");
    }
  } catch (error) {
...
document.getElementById("randomizePlaylistBtn").addEventListener("click", async () => {
  const token = prompt("Ingresa el token para randomizar tu playlist:");
  if (!token) return alert("Se requiere un token.");

  try {
    const response = await fetch(`${baseURL}/api/randomize`, {
      method: "POST",
    ...
    alert("Playlist randomizada con éxito.");
    videos = result.playlist; // Actualizar la playlist en el frontend
    renderPlaylistView(); // Refrescar la vista
  } catch (error) {
    console.error("Error al randomizar la playlist:", error);
  }
});
